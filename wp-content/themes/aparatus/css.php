<?php header('Content-type: text/css'); ?>/* CSS Document */

/* customs */
.fl {float:left;}
.fr {float:right;}
.vh {visibility:hidden;}
.cl {clear:left;}
.b {font-weight:bold;}
.lthr {text-decoration:line-through;}

.bodywrapper {width:1038px;height:auto;margin:auto;}
#header,#wrap,#footer,#navigation,.twitter-entry,#adblock-big,#tabbed-container,.tabbed-container,#adblock-small {float:left;}

#adblock-big {margin-top:10px;width:300px;height:250px;}
#adblock-small {width:300px;}
#page-nav ul {width:770px;float:left;}

.quicklinks {float: left; height: auto; width: 330px;}
.theforums {background-image: url(images/the-forums.jpg);margin-top: 0px; margin-bottom: 0px;}
.theshop { background-image: url(images/the-shop.jpg);margin-top: 10px;}
.themediacenter { background-image: url(images/media-center.jpg);margin-top: 10px;}
.theadvertise { background-image: url(images/advertise.jpg);margin-top: 10px;}
.theforums,.theshop,.themediacenter,.theadvertise {text-indent:999em;background-repeat:no-repeat; background-position:center center; float: left; height: 42px; width: 312px;border: 5px solid rgb(222, 222, 222);}

/* forum ticker */

.forumtickcontainer {font-family:verdana, arial, helvetica;border:1px solid #EEEEEE; float: left; width: 590px; height: 21px;}
.forumlatestposts {padding: 3px; background: transparent url(images/cat-nav-bg.png) repeat-x 0 0; float: left; height: 15px; font-size: 11px; width: 120px;}
.forumticker {padding: 3px; overflow: hidden; float: left; width: 457px; height: 15px; background-color: rgb(255, 255, 255);}
.tickleft {padding:1px 0px;overflow: hidden; float: left; height: 13px; width: 440px;}
.tickleft a {color: rgb(117, 0, 0); text-decoration: none;font-size:10px;}
.tickright {padding:1px 0px;float: left; width: 15px; height: 15px; font-size: 10px; color: rgb(117, 0, 0);}
.tick {float:left;width:458px;}

/* media center */

.media-title-area {background:#EEEEEE none repeat scroll 0 0;border:1px solid #DDDDDD;color:#750000;font-size:48px;margin-bottom:25px;padding:15px 10px 8px;text-transform:capitalize;float:left;}

.media-title-area h3 {font-size:55px;float:left;width:100px;text-transform:uppercase;height:40px;}
.media-title-area p {color:#333333;float:right;font-size:11px;margin-top:0;width:465px;}

.medialinkbox {border:1px solid #DDDDDD;height:auto;padding:10px;width:260px;background-color:#EEEEEE;margin-bottom:20px;}

.medialinks {float:left;background-color:#FFFFFF;border:1px solid #DDDDDD;padding:5px;width:246px;}

.medialinks div {border-bottom:1px dashed #666666;height:15px;line-height:10px;margin-left:20px;width:215px;}

.post-content .medialinkbox .medialinks a.flink {word-wrap: break-word;width:210px;float:left;background:transparent url(images/list-item-style-maroon.gif) no-repeat scroll left center;text-shadow:0;padding-left:20px;clear:both;min-height:20px;}

.post-content .medialinkbox .newsitems a.flink {width:210px;line-height:20px;}

.post-content .newsitems p {line-height:11px;font-size:11px;color:#777777;padding-bottom:0px;float:left;}

.post-content .pressreleases a.flink {width:210px;}

.medialinks div.newsitem {cursor:pointer;height:auto;margin-left:0px;width:auto;border-bottom:0px;}

.post-content .medialinkbox .medialinks span.download,.post-content .medialinkbox .medialinks a.preview,.post-content .medialinkbox .medialinks a.download {float:left;width:auto;color:#666666;background:transparent none;font-size:11px;}
.post-content .medialinkbox .medialinks a.preview {margin-left:20px;}
.post-content .medialinkbox .medialinks a.download,.post-content .medialinkbox .medialinks span.download {margin-left:10px;}

.post-content .medialinkbox h4 {float:left;color:#750000;font-size:30px;text-transform:uppercase}

.post-content a.downloadcontent {background:transparent url(images/download.jpg) no-repeat scroll center 0;float:left;height:137px;text-indent:-1000px;width:245px;}

.post-content a.downloadcontent:hover {background:transparent url(images/download.jpg) no-repeat scroll center -143px;}

/** media center quotes **/

.post-content .quotescontainer h4 a,.post-content .quotescontainer h4 a:hover {background:transparent none;text-shadow:0 none;text-decoration:none;color:#750000;cursor:default;}

.quote {border-bottom-color:#750000;border-bottom-style:dashed;border-bottom-width:1px;float:left;margin-top:25px;height:auto;width:100%;}

.quotesbody p {float:left;width:100%;}

.hires,.lores {font-size:10px;position:absolute;left:2px;}
.hires {margin-right:2px;bottom:14px;}
.lores {bottom:2px;}

/** media center stats **/

.post-content .stats div {padding:10px 0;}

.statsbody p,.statsbody div {float:left;height:auto;padding:10px;width:550px;}

.statsbody .statstitle {background-color:#EEEEEE;;font-size:48px;margin-bottom:25px;padding-bottom:8px;padding-left:10px;padding-right:10px;padding-top:15px;text-transform:capitalize;border:1px solid #DDDDDD}

/** media center advertising form **/

form.advertisingform div,form.advertisingform label, form.advertisingform input[type=text], form.advertisingform textarea {float:left;}
form.advertisingform label {width:190px;}
form.advertisingform div {margin-left:54px;width:500px;height:auto;margin-top:3px;}
form.advertisingform input[type=submit] {border-color:#BBBBBB;color:#464646;background-color:#FFFFFF;margin:auto;}
form.advertisingform select {border: 1px solid rgb(223, 223, 223); padding: 2px; background-color: rgb(255, 255, 255);}

/* facebook */
iframe .fan_box .connections .connections_grid {padding-left:15px;}
.fan_box .full_widget .connect_top {background:#ECEFF5 none repeat scroll 0 0;}

* {padding:0; margin:0;}
body {background:#EDEDED; }
a {outline:none;}
a img {border:none;}

.alignright {float:right;}
.alignleft {float:left;}
.alignnone {clear:both;}
.aligncenter {margin-left: auto; margin-right: auto;}

blockquote {background:url(images/blockquote-bg.png) top left no-repeat; padding:5px 0 0 35px; font-family:georgia,'Lucida Grande','Lucida Sans Unicode'; font-style:italic;}

img.alignright {display:inline;}
img.alignleft {display:inline;}
img.aligncenter {margin:0 auto; display:block;}
img.alignnone {clear:both;}

#header {height:auto;clear:both; padding:70px 49px 0 49px; width:940px; margin:0 auto; text-align:left; background:transparent url(images/header-bg2.jpg) no-repeat 0 22px; position:relative;}
#twitter-badge {position:absolute; left:740px; top:-5px; background:url(images/twitter-badge-maroon.png); width:218px; height:139px;}
#twitter-badge a {display:block; width:218px; height:139px; text-indent:-9999px;}
a#logo {clear:both; /*font-size:98px;*/ overflow:hidden; text-decoration:none; text-transform:uppercase; color:#333333;letter-spacing:-2px; display:inline; margin:0px; padding:0;
line-height:98px;font-size:48px;
}

#wrap {width:940px; padding:0 49px 0 49px; margin:0 auto; text-align:center; background:url(images/wrap-bg.gif) repeat-y; overflow:hidden;}

#navigation {clear:both; background:#B2B2B2 url(images/menu2bg.jpg) no-repeat; height:auto; position:relative; top:-6px;}
#page-nav {height:25px; font-family:verdana, arial, helvetica; font-size:10px; font-weight:bold; padding:0 0 0 0;width:940px;}
#page-nav ul li {float:left; list-style:none; padding:5px 25px 0 10px; display:block;}
#page-nav ul li a {text-decoration:none; color:#FFFFFF;}
#page-nav ul li a:hover {color:#999999;}
#page-nav ul li#welcome {padding-left:10px; padding-right:25px;}
#page-nav ul li#welcome a {color:#000000;}
#page-nav ul li#welcome a:hover {color:#750000;}
#page-nav ul li#welcome.current_page_item2 {background:url(images/page-tab.gif) no-repeat top right #b0b0b0; height:25px; color:#000000;} 
#login {float:right; padding:0; text-transform:uppercase; /*background:url(images/user-silhouette.png) no-repeat; background-position:89px 0;*/}
#login a {color:#000000; text-decoration:none;float:left;margin-right:5px;}
#login a:hover {color:#750000;}
#login a.rss {margin-right: 0px; width: 20px;background:transparent url(images/rss.jpg) no-repeat center center; height: 20px;}
.topfacebook,.toptwitter {width:20px;height:20px;}
.toptwitter {background:url(images/twitter.jpg) no-repeat center center;}
.topfacebook {background:url(images/FaceBook.jpg) no-repeat center center;}

#page-nav ul li.cat-page {background:transparent url(images/cat-nav-bg.png) repeat-x scroll 0 center;border-left:1px solid #999999;border-right:1px solid #999999;border-top:1px solid #999999;margin-left:5px;margin-top:4px;padding:0 10px 1px;text-align:center;}

#page-nav ul li.cat-page a {color:#000000;}
#page-nav ul li.cat-page a:hover {color:#666666;}

.nav2, .nav2 ul, #nav2, #nav2 ul{padding: 0;	float:left;	list-style: none;position:relative;}
.nav2, #nav2 {height:auto;padding:0; float:left;}
.nav2 a, #nav2 a {float:left;height:27px;display:block; font-family:rockwell, "Courier New", Courier,  georgia, arial, helvetica; font-weight:bold; text-transform:uppercase; text-decoration:none; font-size:14px;}
.nav2 li, #nav2 li {height:27px;float:left; line-height:27px;  padding:0px 6px; margin:2px 1px; position:relative;}

.nav2 li a:hover,#nav2 li a:hover {background: none;}
.nav2 li ul,#nav2 li ul {position:absolute;	width: 15em; left: -999em; padding-top:1px;z-index:50;top:90%}
.nav2 li:hover ul, .nav2 li.sfhover ul,#nav2 li:hover ul, #nav2 li.sfhover ul {left:auto; z-index:1000;}
.nav2 li ul.children li,#nav2 li ul.children li {background: #FFFFFF; border-bottom:1px solid #dedbd1; border-left:1px solid #dedbd1;	border-right:1px solid #dedbd1;	line-height:28px; width:15em;position:relative;}
/*.nav2 li:hover, .nav2 li.hover,#nav2 li:hover, #nav2 li.hover {position: static;}*/
.nav2 li ul ul,#nav2 li ul ul {margin: -29px 0 0 15em;}

.nav2 li:hover ul ul, .nav2 li.sfhover ul ul,#nav2 li:hover ul ul, #nav2 li.sfhover ul ul {left: -999em;}
.nav2 li:hover ul, .nav2 li li:hover ul, #nav2 li.sfhover ul, #nav2 li li.sfhover ul {left: auto;}
.nav2 li:hover ul ul, .nav2 li:hover ul ul ul, .nav2 li.sfhover ul ul, .nav2 li.sfhover ul ul ul, #nav2 li:hover ul ul, #nav2 li:hover ul ul ul, #nav2 li.sfhover ul ul, #nav2 li.sfhover ul ul ul {left: -999em;}
.nav2 li:hover ul, .nav2 li li:hover ul, .nav2 li li li:hover ul, .nav2 li.sfhover ul, .nav2 li li.sfhover ul, .nav2 li li li.sfhover u, #nav2 li:hover ul, #nav2 li li:hover ul, #nav2 li li li:hover ul, #nav2 li.sfhover ul, #nav2 li li.sfhover ul, #nav2 li li li.sfhover ul {left: auto;} 


.cat-nav,#cat-nav {clear:both; padding:0px 7px 7px;width:926px;height:auto;display:inline-block;}
.cat-nav ul li.cat-item,#cat-nav ul li.cat-item {background:url(images/cat-nav-bg.png) repeat-x; border-top:1px solid #999;  border-right:1px solid #999;  border-left:1px solid #999;}

.cat-nav ul li.cat-item a,#cat-nav ul li.cat-item a {color:#750000; text-shadow:0 1px 0 #fff;}
.cat-nav ul li.cat-item a:hover,#cat-nav ul li.cat-item a:hover {color:#000000;}
.cat-nav ul li.cat-item.current-cat,#cat-nav ul li.cat-item.current-cat {background:url(images/cat-nav-current-bg.png) repeat-x;} 
.cat-nav ul li.cat-item.current-cat a ,#cat-nav ul li.cat-item.current-cat a {color:#FFFFFF; text-shadow:none;}
.nav2 li ul.children li.cat-item.current-cat,#nav2 li ul.children li.cat-item.current-cat {background:#ffffff;} 
.nav2 li ul.children li.cat-item.current-cat a,#nav2 li ul.children li.cat-item.current-cat a {color:#750000; text-shadow:0 1px 0 #fff;} 
.cat-nav ul li.cat-item.current-cat ul.children li a,#cat-nav ul li.cat-item.current-cat ul.children li a {color:#750000; text-shadow:0 1px 0 #fff;}

/*single*/
.main-container{width:590px; overflow:hidden; margin:20px 20px 20px 0; float:left; text-align:left; font-family:Arial, Helvetica, sans-serif; font-size:13px; position:relative;}
.main-container a {color:#750000; text-decoration:none;}
.main-container a:hover {color:#666666; text-decoration:underline;} 

.title-area {padding:15px 10px 8px 10px; color:#750000; font-size:48px; text-transform:capitalize; background:#EEEEEE; border:1px solid #DDDDDD; margin-bottom:25px; }


.content {position:relative;}

.retweet-btn-small {float:right; position:relative; top:6px; left:22px;}
.retweet-btn {float:right; padding:2px 3px 0 5px;}
.post-title {font-family:helvetica, arial; letter-spacing:-1px; font-size:24px; font-weight:bold; line-height:26px; margin-bottom:15px; text-transform:capitalize;}
.post-title a {text-decoration:none; color:#333333;}
.post-title a:hover {color:#750000; text-decoration:underline;}
.post-title-big {font-size:48px; font-weight:normal; line-height:38px; margin-bottom:15px; text-transform:capitalize; /*text-indent:-8px;*/}
.post-title-big a {text-decoration:none; color:#333333;}
.post-title-big a:hover {text-decoration:none;}
.post-meta-data {font-size:11px; color:#434343; text-transform:uppercase; font-family:georgia; margin-bottom:15px; }
img.thumbs-in-archive {float:right; padding:5px; border:1px solid #CCCCCC; margin:1px 0 0 15px;}
.post-excerpt {line-height:20px; height:85px; overflow:hidden; margin-bottom:22px;}
.post-content {clear:both; line-height:24px; margin-bottom:30px; overflow:hidden;}
.post-content a {background:#EEEEEE; text-shadow:0 1px 0 #fff;}
.post-content ol {padding:10px 0 10px 40px;}
.post-content ol li ol {padding-left:20px;}
.post-content ol li ul {padding-left:20px;}
.post-content ul {padding:10px 0 10px 40px;}
.post-content ul li ol {padding-left:20px;}
.post-content ul li ul {padding-left:20px;}
.post-content p {padding:0 0 15px 0;}
.post-content h1, .post-content h2, .post-content h3, .post-content h4, .post-content h5, .post-content h6 {font-size:24px; font-weight:normal; line-height:30px; padding-bottom:10px;}

.wp-caption {padding:8px 3px 0px 3px; border:1px solid #eeeeee; text-align:center; margin:0 auto;}
.wp-caption.alignright {margin:8px 0 5px 15px;} 
.wp-caption.alignleft {margin:8px 15px 5px 0;}
.wp-caption.aligncenter{clear:both; margin:15px auto; display:block; }
.wp-caption.alignnone {clear:both; margin:15px auto; display:block; }
p.wp-caption-text {text-align:center; font-family:georgia; font-style:italic; font-size:11px; line-height:13px;}

.post-content img.alignright {display:inline; margin:8px 0 5px 20px;}
.post-content img.alignleft {display:inline; margin:8px 20px 5px 0;}
.post-content img.aligncenter {clear:both; margin:10px auto; display:block; }
.post-content img.alignnone {clear:both; margin:10px auto; display:block; }
.post-content a img.alignright {display:inline; margin:8px 0 5px 20px;}
.post-content a img.alignleft {display:inline; margin:8px 20px 5px 0;}
.post-content a img.aligncenter {clear:both; margin:10px auto; display:block; }
.post-content a img.alignnone {clear:both; margin:10px auto; display:block; }

.post-content p img.alignright {display:inline; margin:20px 0 5px 20px;}
.post-content p img.alignleft {display:inline; margin:20px 20px 5px 0;}
.post-content p img.aligncenter {clear:both; margin:10px auto; display:block; }
.post-content p img.alignnone {clear:both; margin:10px auto; display:block; }
.post-content p a img.alignright {display:inline; margin:8px 0 5px 20px;}
.post-content p a img.alignleft {display:inline; margin:8px 20px 5px 0;}
.post-content p a img.aligncenter {clear:both; margin:10px auto; display:block; }
.post-content p a img.alignnone {clear:both; margin:10px auto; display:block; }

#page-links {font-family:verdana, rockwell, "Courier New", Courier,  georgia, arial, helvetica; clear:both; margin:10px 0; font-weight:bold; text-transform:uppercase; font-size:11px; margin:5px 0 30px 0; clear:both;}
#page-links a {padding:2px 4px;  text-decoration:none; color: #ffffff; background-color: #750000; border: 1px solid #750000; text-shadow:none; }
#page-links a:hover {border: 1px solid #750000; background:#FFFFFF; color:#333333;}

.author-box { background:#eee; border:1px solid #DDDDDD; padding:20px; margin-bottom:30px;}
.photo,.author-box img.avatar {float:left; border:1px solid #ccc; /* padding:3px; */ margin:1px 10px 0 0;}
.author-box .author-name {padding-bottom:0px; font-style:italic; font-weight:bold; font-size:24px; color:#750000;}
.author-box .author-description {font-family:verdana, arial, helvetica; font-size:11px; line-height:16px; color:#666666;}
.author-box .author-links {margin:14px 0 2px; font-family:georgia, arial, helvetica; text-transform:uppercase; font-size:11px; border-top:1px dashed #ccc; padding-top:14px;} 
.author-box-small { background:#eee; border:1px solid #DDDDDD; padding:5px 10px 5px 5px; text-align:right; font-family:verdana, arial, helvetica; font-size:11px; margin-bottom:25px; text-shadow:0 1px 0 #fff;}

.social-bar {background:url(images/spread-the-word.png) no-repeat top left; height:63px; text-align:right; margin:0 0 30px 0; position:relative;/* added by LP */ float:left;height:60px;width:585px;clear:both;}
ul#social-btns {position:absolute; left:299px;/** added by LP */ width:290px;display:inline;float:left;}
ul#social-btns li {float:left; height:63px; list-style:none; /* added by LP */ float:left;height:63px;display:inline;}
ul#social-btns li a {height:63px; background:url(images/social-sprites.png); display:block; text-indent:9999px;}
ul#social-btns li a#delicious {width:68px; background-position:0 -66px;}
ul#social-btns li a:hover#delicious {width:68px; background-position:0 0;}
ul#social-btns li a#digg {width:52px; background-position:-68px -66px;}
ul#social-btns li a:hover#digg {width:52px; background-position:-68px 0;}
ul#social-btns li a#stumbleupon {width:58px;  background-position:-120px -66px;}
ul#social-btns li a:hover#stumbleupon {width:58px;  background-position:-120px 0;}
ul#social-btns li a#technorati{width:58px; background-position:-178px -66px;}
ul#social-btns li a:hover#technorati{width:58px; background-position:-178px 0;}
ul#social-btns li a#twitter {width:54px; background-position:-236px -66px;} 
ul#social-btns li a:hover#twitter {width:54px; background-position:-236px 0;} 
/*comments*/

#comments {padding:0 0 10px 0; }
#comments h3{text-align:left; font-size:30px; font-weight:normal; line-height:38px; color:#333333;}
img.avatar {float:left; padding:3px; border:1px solid #ccc; margin:0 10px 0 0;}
cite.fn a {font-family:georgia; font-style:normal; text-transform:uppercase; padding:0; color:#333333; text-decoration:none; font-size:11px;}
.comment-author {padding:0; line-height:14px;}
.comment-meta {font-family:'Trebuchet MS', verdana;  font-size:11px; color:#666666; line-height:14px; padding-bottom:5px;}
.comment-meta a {color:#750000; font-weight:normal; text-decoration:none;}
.comment-body {margin-bottom:20px; padding:20px 20px 5px 20px; font-style:italic; font-family:'Lucida Grande','Lucida Sans Unicode','Trebuchet MS',verdana,georgia; overflow:hidden;}

.comment-body p a {color:#750000; font-weight:bold; text-decoration:none;}
.comment-body p a:hover{text-decoration:underline;}
li.comment.even {background:#fff; border:1px solid #DDDDDD; margin-bottom:20px;}
li.comment.odd {background:#eee; border:1px solid #DDDDDD; margin-bottom:20px;}
ul.commentlist li {list-style:none;}
li.parent {padding-bottom:10px;}
.commentlist ul.children {margin:20px;}
.comment-meta .commentmetadata p a {color:#21759B; }
.reply {clear:both; border-top:1px dashed #ccc; padding-top:7px; margin-top:12px; text-align:right;}
a.comment-reply-link {color:#21759B; font-family:verdana; font-size:11px; padding:0 0 0 19px; background:url(images/arrow-135-medium.png) left no-repeat; }

/*response*/
h3#response-title {text-align:left; font-size:30px; font-weight:normal; line-height:38px; color:#333333; padding:0 15px 35px 0; }
h3#response-title a {color:#000000; text-decoration:none; text-transform:capitalize;}
a#cancel-comment-reply-link {padding:5px; background:#ab1515; color:#FFFFFF;  font-family:rockwell, georgia; text-transform:uppercase; text-decoration:none;}
#commentform {padding:45px 25px 25px 25px; background:#fff; border:1px solid #DDDDDD; margin-bottom:25px;}
#commentform p {padding-bottom:5px;}
.comment-form-input-fields {padding:5px; margin-bottom:10px;}
.comment-form-labels {font-family:rockwell,georgia; font-weight:bold; color:#FFFFFF; width:162px; font-size:14px; padding:5px; text-transform:uppercase; background:url(images/comment-labels.gif) no-repeat top right #2b2b2b; }
.comment-form-labels small {font-family:Verdana, Arial, helvetica; font-size:9px; font-weight:normal;}
input.send-comment {background:#ab1515; color:#FFFFFF; font-family:rockwell, georgia; padding:5px;}

/*snarfer specific*/

h3.tab-bars {clear:both;  background:url(images/cat-nav-bg.png) repeat-x; height:22px; border:1px solid #e9e9e9; padding:5px 0 0 7px; margin-bottom:25px; } 
h3.tab-bars a {font-family:rockwell, "Courier New", Courier,  georgia, arial, helvetica; font-weight:bold; text-transform:uppercase; text-decoration:none; line-height:16px; font-size:14px; text-shadow:0 1px 0 #fff; color:#750000;}
h3.tab-bars a:hover {text-shadow:none; color:#000000;}
.post-container {width:285px; float:left;}
.post-container.odd {margin-right:20px;}
.post-container .post-title-sm {font-size:17px; padding-bottom:10px; text-transform:capitalize; font-weight:bold;}
.post-container .post-title-sm a {text-decoration:none; color:#333333; }
.post-container .post-title-sm a:hover {text-decoration:underline; color:#750000;}
.post-container .post-meta-data {font-size:11px; color:#434343; text-transform:uppercase; font-family:georgia; padding:0 0 5px 0; margin:0;}
.post-container img.thumbs {padding:5px; border:1px solid #CCCCCC; margin-bottom:10px;}
.post-container .post-excerpt-small {overflow:hidden; height:65px; font-size:12px; line-height:20px; margin-bottom:10px;}
.post-container .continue {background:#eee; border:1px solid #DDDDDD; padding:5px 10px 5px 5px; text-align:right; font-family:verdana, arial, helvetica; font-size:11px; margin-bottom:25px;}
.post-container .continue a {font-family:verdana, arial, helvetica; font-weight:bold; font-size:10px; color:#666666; text-decoration:none; text-shadow:0 1px 0 #fff; }
.post-container .continue a:hover {text-decoration:underline;}
#view-all {float:right; padding: 2px 4px; margin:20px 0 20px 0; background:#eee; border:1px solid #DDDDDD;}
#view-all a {font-family:verdana, rockwell, "Courier New", Courier,  georgia, arial, helvetica; clear:both; margin:10px 0; font-weight:bold; text-transform:uppercase; font-size:11px; color: #000000; text-decoration:none; text-shadow:0 1px 0 #fff;}
#view-all a:hover {text-decoration:underline;}

/*sidebar*/
#sidebar {width:330px; min-height:700px; float:left;}
#searchform {background:#E5E5E5 none;border-bottom:1px solid #DDDDDD;border-left:1px solid #DDDDDD;border-right:1px solid #DDDDDD;height:40px;text-align:left;width:319px;margin-top:-25px}
input.inputs {margin:11px 0 0 12px;}
input.go {background:transparent url(images/cat-nav-bg.png) repeat-x 0 center; color:#750000;}
.twitter-entry {margin-top:10px;background:url(images/twitter.gif); width:331px; height:91px;}
.twitter-entry p {font-family:verdana, helvetica, arial; font-size:11px; text-align:left; padding:20px 5px 0 100px;  text-shadow:0 1px 0 #fff;}

.twitter-entry p {background:#FFFFFF none;height:63px;margin-left:100px;margin-top:10px;padding:5px 10px;text-align:left;text-shadow:0 1px 0 #FFFFFF;width:205px;}

.twitter-entry a,#twitter-entry a {color:#750000; text-decoration:none;}
.twitter-entry a:hover,#twitter-entry a:hover {text-decoration:underline;}
#adblock-big {padding:15px 10px 15px 10px; background:#e5e5e5; margin-bottom:10px; border:1px solid #DDDDDD;}
#adblock-small {border:1px solid #DDDDDD; clear:both; background:#ededed; padding:25px 2px 25px 26px;/*20px 0 10px 28px; */overflow:hidden; margin:10px 0 10px 0;}

#adblock-small ul {background-color:#FFFFFF;float:left;height:auto;width:275px;}

#adblock-small ul li {float:left; list-style:none; padding:/*10px 24px 10px 0;*/5px;}

#adblock-small ul li img {border:1px solid #750000;height:125px;width:125px;}

.sidebar-row{padding:25px 20px; background:#e5e5e5; margin-bottom:20px; border:1px solid #DDDDDD; text-align:left; font-family:verdana,arial,helvetica; font-size:11px; line-height:16px; text-shadow:0 1px 0 #fff;}
.sidebar-row h3 {font-size:32px; padding-bottom:10px;}
.sidebar-row ul {padding:0 10px 0 10px;}
.sidebar-row ul li{list-style:none; padding:6px 0 6px 0; border-bottom:1px dashed #CCCCCC; background:url(images/list-item-style-maroon.gif) no-repeat; background-position:0 9px; padding-left:20px;}
.sidebar-row ul.children li {border-bottom:none;}
.sidebar-row a {color:#000000; text-decoration:none;}
.sidebar-row a:hover {text-decoration:underline; color:#750000;}
.sidebar-row #calendar_wrap {background:#ffffff; border:1px solid #DDDDDD; padding:25px;}
.sidebar-row #calendar_wrap caption {font-weight:bold; padding-bottom:25px; font-size:20px; font-family:helvetica; text-shadow:0 1px 0 #fff;}
.sidebar-row table#wp-calendar {border-collapse:collapse; width:100%; text-align:center;}

/** text links **/

.textlink-container {border:1px solid #DDDDDD; clear:both; background:#ededed;  padding:16px 0 20px 16px; height:auto; margin-bottom:20px;text-align:left;float:left;width:310px; color:#666666; font-family:verdana,arial,helvetica; font-size:11px; line-height:16px;}
.textlink-container h3 {color:#750000;width:100%;float:left;height:auto;}
.textlink-container div {padding: 10px; float: left; width: 260px; background-color:#FFFFFF;}
.textlink-container a:hover,.textlink-container a {color:#750000;width:100%;background:transparent url(images/list-item-style-maroon.gif) no-repeat left 4px;float:left;height:20px;padding-left:20px;width:240px;}
.textlink-container p {border-bottom:1px dashed #CCCCCC;float:left;padding-bottom:5px;padding-left:20px;width:240px;}

/*tabbed-menu*/
#tabbed-container,.tabbed-container {border:1px solid #DDDDDD; clear:both; background:#ededed;  padding:16px 0 20px 16px; height:364px; margin-bottom:10px; text-align:left; color:#666666; font-family:verdana,arial,helvetica; font-size:11px; line-height:16px;width:310px;}
ul.mootabs_title {list-style-image: none; list-style-type: none; padding: 0px;	height: 24px; font-family:rockwell, "Courier New", Courier,  georgia, arial, helvetica; font-weight:bold; text-transform:uppercase; color:#750000; text-decoration:none; line-height:16px; font-size:14px; letter-spacing:-1px; text-shadow:0 1px 0 #fff;}
ul.mootabs_title li {float: left; display:block; padding:5px 7px 0 7px; height:22px; line-height:16px; margin-right:4px; cursor: pointer; background:url(images/cat-nav-bg.png) repeat-x; border-top:1px solid #DDDDDD;  border-right:1px solid #DDDDDD;  border-left:1px solid #DDDDDD;  border-bottom:1px solid #CCCCCC;}
.mootabs_title li.active { background:url(images/cat-nav-current-bg.png); color:#FFFFFF; text-shadow:none; border:1px solid #750000; }
.mootabs_panel {display: none; position: relative; width:100%; top: 12px; clear: both;	overflow: hidden;}
.mootabs_panel p {font-family:Arial, Helvetica, sans-serif; padding:25px; text-align:left;}
.mootabs_panel.active { background-color: #fff; display: block;}

/*popular posts*/
#Popular {padding:15px 25px 25px 25px;  width:240px; border:1px solid #DDDDDD;}
#Popular a{text-decoration:none; color:#666666;}
#Popular a:hover{text-decoration:underline; color:#000000;}
#Popular ul{padding:0 10px 0 0;}
#Popular ul li {list-style:none; padding:6px 0 6px 0; border-bottom:1px dashed #CCCCCC; background:url(images/list-item-style-maroon.gif) no-repeat; background-position:0 9px; padding-left:20px;}
/*recent comments*/
#RecentComments {padding:15px 25px 25px 25px;  width:240px; font-weight:bold; color:#000000; border:1px solid #DDDDDD;}
#RecentComments a{text-decoration:none; color:#666666; font-weight:normal; padding-left:5px;}
#RecentComments a:hover{text-decoration:underline; color:#000000;}
#RecentComments ul{padding:0 10px 0 0;}
#RecentComments ul li {list-style:none; padding:6px 0 6px 0; border-bottom:1px dashed #CCCCCC; background:url(images/list-item-style-maroon.gif) no-repeat; background-position:0 9px; padding-left:20px;}
/*tag cloud*/
#Tags {padding:15px 25px 25px 25px;  width:240px; border:1px solid #DDDDDD;}
#Tags a {text-decoration:none; color:#666666;}
#Tags a:hover {text-decoration:underline; color:#000000;}

/*page-navi*/

.wp-pagenavi {font-family:verdana, rockwell, "Courier New", Courier,  georgia, arial, helvetica; clear:both; margin:10px 0; font-weight:bold; text-transform:uppercase; font-size:11px; float:right;}
.wp-pagenavi a, .wp-pagenavi a:link { padding:2px 4px; margin: 2px;text-decoration: none;	border: 1px solid #750000; color: #ffffff; background-color: #750000;}	
.wp-pagenavi a:visited {padding:2px 4px; margin: 2px;text-decoration: none;	border: 1px solid #750000; color: #ffffff; background-color: #750000;}
.wp-pagenavi a:hover {border: 1px solid #750000; color: #000000; background-color: #FFFFFF;}
.wp-pagenavi a:active {padding:2px 4px; margin:2px; text-decoration: none;	border: 1px solid #750000; background-color: #FFFFFF;}
.wp-pagenavi span.pages {padding: 2px 4px; margin: 2px;	color: #000000;	background:#eee; border:1px solid #DDDDDD; text-shadow:0 1px 0 #fff;}
.wp-pagenavi span.current {padding: 2px 4px; margin: 2px; font-weight: bold; background:#eee; border:1px solid #DDDDDD;}
.wp-pagenavi span.extend {padding: 2px 4px 2px 4px; margin: 2px; border: 1px solid #000000;	color: #000000;	background-color: #FFFFFF;}
.wp-pagenavi span.current a.page{color:#FFFFFF; }
.next-prev-links {font-family:verdana, rockwell, "Courier New", Courier,  georgia, arial, helvetica; clear:both; margin:10px 0; font-weight:bold; text-transform:uppercase; font-size:11px; float:right;}
.next-prev-links a {padding:2px 4px; margin: 2px;text-decoration: none;	border: 1px solid #750000; color: #ffffff; background-color: #750000;}




/*footer*/
#footer {height:106px; clear:both; padding:40px 49px 0 49px; width:940px; margin:0 auto; text-align:left; background:url(images/footer-bg.gif) no-repeat top left; position:relative;
}
#logo-footer {text-transform:uppercase; color:#333333; font-size:30px; display:block; margin:18px 7px 2px 0; padding:0; clear:both; text-align:right;}
#page-nav-footer {float:left; text-align:right;}
#page-nav-footer ul li {float:left; list-style:none; line-height:8px; padding:0 8px 2px 8px; margin-bottom:3px; background:url(images/credits-sep.png) no-repeat top left;}
#page-nav-footer ul li.first {background:none;} 
#page-nav-footer ul li a {font-family:verdana, helvetica, arial; font-size:11px; color:#750000; text-decoration:none; text-shadow:0 1px 0 #fff;}
#page-nav-footer ul li a:hover {text-decoration:underline;}
#credits {display:block; padding:2px 7px 0 0; clear:both; font-family:georgia; font-size:11px; text-shadow:0 1px 0 #fff; display:block; text-align:right;}
#credits a{color:#000000; text-decoration:none;}
#credits a:hover {color:#666666;}


/** upcoming Events list **/

.upcomingcontainer {font-family:Arial,Helvetica,sans-serif;background-color:#EDEDED;border:1px solid #DDDDDD;float:left;height:auto;padding:10px 13px;width:293px;margin-top:10px;}
h4.upcoming {color:#750000;float:left;}
.upcominglist {background-color:#FFFFFF;border:1px solid #DDDDDD;float:left;height:auto;margin-top:10px;padding:5px 15px;width:260px;}
.event {float:left;width:260px;height:auto;text-align:left;border-bottom:1px dashed #750000;margin-bottom:10px;}
.event a.lnk {text-decoration:none;background:transparent url(images/list-item-style-maroon.gif) no-repeat 0 0;color:#750000;float:left;font-size:12px;font-weight:bold;height:auto;padding-left:20px;width:230px;}
.event p {float:left;font-size:12px;padding:5px 5px 10px 18px;}
.event p a {color:#750000}
.event span {color:#5D5D5D;float:left;font-size:11px;margin:2px 0 0 18px;width:200px;}

/** contribute **/
.main-container a.contributelarge-cat {margin-bottom:30px;}
.post-content a.contributelarge-pagepost {margin-bottom:5px;margin-top:10px;}
.main-container a.contributelarge-cat,.post-content a.contributelarge-pagepost {background:transparent none;border-bottom:1px dashed #750000;border-top:1px dashed #750000;float:left;height:64px;text-align:center;width:590px;padding-bottom:10px;padding-top:10px;}

/** Book of the Month **/
.botm {float:left;background:#EDEDED none repeat scroll 0 0;border:1px solid #DDDDDD;clear:both;font-family:verdana,arial,helvetica;font-size:11px;height:auto;line-height:16px;margin-bottom:10px;padding:16px 0 20px 16px;text-align:center;width:310px;}
.botm div {float:left;width:240px;height:auto;text-align:center;}
.botm .w {background-color:#FFFFFF;border:1px solid #DDDDDD;padding:15px 25px 25px;width:240px;}
.botm h3 {color:#750000;font-size:20px;width:300px;text-align:left;}
.botm a {color:#000000;text-decoration:none;width:240px;height:auto;text-align:center;float:left;}
.botm a:hover {color:#666666;text-decoration:underline;}

.supportus {  background: url(images/yourparlour.jpg) no-repeat scroll 0 -55px transparent;
    border: 2px solid #750000;
-moz-border-radius:8px;
-webkit-border-radius:8px;
border-radius:8px;
    float: left;
    height: 96px;
    margin-bottom: 20px;
    padding: 15px 15px 25px 285px;
    width: 282px;
cursor:pointer
}

.supportus p.supportyou {font-size:35px;color:#666666;}

.supportus p.supportyou span {font-size:40px;color:#888888}

.supports p.support {font-size:26px;color:#750000;padding-top:5px;}

a.support:hover,a.support {font-size:30px;color:#750000;border-bottom:1px dotted #750000;text-decoration:none}
.supports span {font-size:18px;font-family:Arial;color:#474747}

.post-content a.shout, .shout {
background: url(images/shout.jpg) no-repeat scroll 0 0 transparent;
float: left;
height: 140px;
margin-bottom: 20px;
width: 586px;
cursor:pointer
}


#block-big {margin-top:10px;width:300px;height:250px;}
#block-small {width:300px;}
#header,#wrap,#footer,#navigation,.twitter-entry,#block-big,#tabbed-container,.tabbed-container,#adblock-small {float:left;}

#block-big {padding:15px 10px 15px 10px; background:#e5e5e5; margin-bottom:10px; border:1px solid #DDDDDD;}
#block-small {border:1px solid #DDDDDD; clear:both; background:#ededed; padding:25px 2px 25px 26px;/*20px 0 10px 28px; */overflow:hidden; margin:10px 0 10px 0;}

#block-small ul {background-color:#FFFFFF;float:left;height:auto;width:275px;}

#block-small ul li {float:left; list-style:none; padding:/*10px 24px 10px 0;*/5px;}

#block-small ul li img {border:1px solid #750000;height:125px;width:125px;}

